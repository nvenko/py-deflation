...
for __smp in ("mc", "mcmc"):
  for which_op in ("previous", "current"):
    for kl_strategy in range(2):
      ...
      dcgmo[(__smp, which_op, kl_strategy)] = recycler(smp[__smp], __dcg, "dcgmo", kl=kl, kl_strategy=kl_strategy, which_op=which_op, t_end_kl=300, t_end_def=300)
...
lw = 0.3
fig, ax = pl.subplots(2, 3, figsize=(13.5,7), sharex="col")
...
for j in range(3):
  ax[0,j].set_ylim(ax[1,j].get_ylim())
  ax[1,j].set_xlabel("Realization index, t")
